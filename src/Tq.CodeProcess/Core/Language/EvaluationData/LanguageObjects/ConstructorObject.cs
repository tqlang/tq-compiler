...
public class ConstructorObject(ConstructorDeclarationNode synNode) : LangObject("constructor"),
        IDotnetImportMethodModifier
{
    ...
    public override string ToString()
    {
        ...
        sb.Append($"ctor({string.Join(", ", _parameters.Select(e => e.Type))})");
    ...
    public override string ToSignature() => $"{Parent:sig}" +
                                   $".ctor({string.Join(", ", _parameters.Select(e => e.Type))})";
}
